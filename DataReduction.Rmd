...
Specify which type of template is in each of the arrays.  
```{r}
#pSch = "PSwap"
#allFluors$Scheme <- pSch
...
EITHER Calculate thresholds using method 1: using previously determined maxSD of negative controls
```{r calculate thresholds Method 1}
...
Visualize amp thresholds to spot errors
```{r view amp thresh}
...
```{r save box plots}
ggsave(FinalZygosity_PSwap, filename = paste0(output,"/", runDate, "FinalZygosity_PSwap.jpeg"))
```
...
Melt the individual run CellOutputData together to make snowConeCells
```{r going on a bear hunt}
